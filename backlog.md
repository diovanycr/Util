...
- [ ] **#B1 — `terminalFutura.js` ainda existe mas não é mais usado**
...
- [ ] **#B2 — Atalho numérico `4` (Sistemas) não está em `enhancements.js`**
...
- [ ] **#B3 — `messagesInitialized` e `problemsInitialized` nunca são resetados corretamente no logout**
...
- [ ] **#B4 — `exportToJson` inclui `deleted` e `order` mas exclui apenas `id`, `createdAt`, `updatedAt`**
...
- [ ] **#B5 — `saveOrder` em `messages.js` pode falhar silenciosamente quando o drag cruza grupos de categoria**
...
- [ ] **#B6 — Busca em `enhancements.js` usa `#globalSearch` que não existe no HTML**
...
- [ ] **#B7 — Favorites são armazenados em `localStorage` por ID do Firestore**
...
- [ ] **#B8 — `problem-copy-field:hover` usa cor hardcoded `#eff6ff` não compatível com dark mode**
...
- [ ] **#10 — Empty state ausente nas listas (Mensagens e Problemas)**
...
- [ ] **#14 — Modo compacto sem indicação de estado ativo no botão**
...
- [ ] **#15 — Admin: exclusão de usuário não deleta links no Firestore**
...
- [ ] **#16 — Histórico de cópias limita a 20 itens mas nunca notifica o usuário**
...
- [ ] **#17 — `problems.css` usa cor hardcoded `#166534` para `.solution-text`**
